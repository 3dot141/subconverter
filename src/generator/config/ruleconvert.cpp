...
std::string convertRuleset(const std::string &content, int type)
{
    ...
    {
        // 如果有一行的格式为 host-wildcard，则这一行都置为空
        output = regReplace(content, "^(?i:host-wildcard).*$", "", true, true);
        // read: 处理 quanx 的配置和别人的配置不一样
        output = regReplace(regReplace(output, "^(?i:host)", "DOMAIN", true), "^(?i:ip6-cidr)", "IP-CIDR6", true); //translate type
